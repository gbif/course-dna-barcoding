...
. Select a https://training-ipt-b.gbif.org/resource?r=vicia_ar_ge[dataset^] published on the IPT and review the metadata.
. Download the Darwin Core Archive.
. Navigate to the dataset on GBIF.
. Review the occurrence records through the GBIF dev portal.
...
. Download link:../exercise-data/RBGE-occurrence.txt[RBGE-occurrence.txt,opts=download] (ZIP 76 KB)
. Download link:../exercise-data/RBGE-multimedia.txt[RBGE-multimedia.txt,opts=download] (ZIP 65 KB)
