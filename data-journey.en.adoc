...
. Download link:../exercise-data/ViciaForCleaning.txt[ViciaForCleaning.txt file,opts=download] (ZIP 66 KB)
. Import the file in Excel using the Excel wizard. See https://docs.gbif.org/course-data-mobilization/course-docs/Excel-tips-EN.pdf[Excel-tips-EN.pdf^] (PDF, 7 MB) for import instructions for your operating system (Windows, Mac, Linux).
